...
run() {
  group('Persistent set', () {
    ...
    test('difference', () {
      ...
      expect(set1.difference(set2) == new PSet.from(["a", "b"]), isTrue);
    });
  ...
  group('Transient set', () {
    ...
    test('doInsert', () {
      var set = new PSet.from(["a", "b", "c"]).asTransient();
      set.doInsert("c");
      expect(set.asPersistent(), equals(new PSet.from(["a", "b", "c"])));
      expect(set.contains("c"), isTrue);
      set = set.asPersistent().asTransient();
      set.doInsert("d");
      expect(set.asPersistent(), equals(new PSet.from(["a", "b", "c", "d"])));
      expect(set.contains("d"), isTrue);
    });
